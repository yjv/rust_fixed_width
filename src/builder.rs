...
use spec::{FileSpec, DataRecordSpecRecognizer, NoneRecognizer};
...
pub struct FileBuilder<'a, T: File, U: DataRecordSpecRecognizer> {
    pub file: T,
    spec: &'a FileSpec,
    recognizer: U
}

impl<'a, T: File, U: DataRecordSpecRecognizer> FileBuilder<'a, T, U> {
    pub fn new(file: T, spec: &'a FileSpec) -> FileBuilder<'a, T, NoneRecognizer> {
        FileBuilder { file: file, spec: spec, recognizer: NoneRecognizer }
    }

    pub fn new_with_recognizer(file: T, spec: &'a FileSpec, recognizer: U) -> Self {
        FileBuilder { file: file, spec: spec, recognizer: recognizer }
    }

    pub fn add_line<V: AsRef<HashMap<String, String>>>(&'a mut self, data: V, spec_name: Option<String>) -> Result<usize, Error<T>> {
        let record_spec_name = try!(spec_name.or_else(|| self.recognizer.recognize_for_data(&data, &self.spec.record_specs)).ok_or(Error::RecordSpecNameRequired));
