...
use spec::{FileSpec, DataRecordSpecRecognizer, ErrorFieldRecognizer};
...
impl<'a, T: File, U: DataRecordSpecRecognizer> FileBuilder<'a, T, U> {
    pub fn new(file: T, spec: &'a FileSpec) -> FileBuilder<'a, T, ErrorFieldRecognizer> {
    ...
    pub fn add_line<V: AsRef<HashMap<String, String>>>(&'a mut self, data: V, spec_name: Option<String>) -> Result<usize, Error<T, U>> {
        let record_spec_name = try!(spec_name.map_or_else(
            || self.recognizer.recognize_for_data(&data, &self.spec.record_specs).map_err(Error::FailedToRecognizeRecordSpec),
            |name| Ok(name))
        );
