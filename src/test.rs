...
pub struct MockRecognizer<'a, T: DataRanges + 'a = ()> {
    ...
    data_recognize_calls: Vec<(&'a Data<T>, &'a HashMap<String, RecordSpec>, Result<String, ::recognizer::Error>)>
}
...
impl<'a, T: DataRanges + 'a = ()> MockRecognizer<'a, T> {
    ...
    pub fn add_data_recognize_call(&mut self, data: &'a Data<T>, record_specs: &'a HashMap<String, RecordSpec>, return_value: Result<String, ::recognizer::Error>) -> &mut Self {
...
impl<'a, T: DataRanges + 'a = ()> DataRecordSpecRecognizer for MockRecognizer<'a, T> {
    fn recognize_for_data<U: DataRanges>(&self, data: &Data<U>, record_specs: &HashMap<String, RecordSpec>) -> Result<String, ::recognizer::Error> {
        for &(ref expected_data, ref expected_record_specs, ref return_value) in &self.data_recognize_calls {
            if *expected_data as *const Data<T> == data as *const Data<U> as *const Data<T>
